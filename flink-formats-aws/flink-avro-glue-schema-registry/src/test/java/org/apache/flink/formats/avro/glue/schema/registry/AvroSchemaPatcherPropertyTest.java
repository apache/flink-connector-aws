...
class AvroSchemaPatcherPropertyTest {
    ...
    private Arbitrary<RowType> rowTypesWithDepth(int depth) {
        ...
        return fieldCount.flatMap(
                count -> {
                    Arbitrary<List<LogicalType>> types =
    ...
    private Arbitrary<LogicalType> logicalTypesWithDepth(int depth) {
        ...
        Arbitrary<LogicalType> primitives =
                Arbitraries.of(
        ...
        Arbitrary<LogicalType> arrayType = logicalTypesWithDepth(depth + 1).map(ArrayType::new);

        Arbitrary<LogicalType> mapType =
        ...
        Arbitrary<LogicalType> nestedRowType =

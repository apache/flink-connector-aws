...
public class GlueCatalog extends AbstractCatalog {
    ...
    private static GlueClient createClient(Properties glueClientProperties) {
        return AWSClientUtil.createAwsSyncClient(
                ...
                AWSGeneralUtil.createSyncHttpClient(glueClientProperties, ApacheHttpClient.builder()),
                GlueClient.builder(),
    ...
    public void close() throws CatalogException {
        try {
            ...
            LOG.warn("Glue Client is not closed properly!", e);
        }
    }
        ...
        if (databaseExists(databaseName)) {
            if (cascade) {
                ...
                if (!tables.isEmpty()) {
                    ...
                    LOG.info("All Tables deleted from Database {}.", databaseName);
                }
                ...
                if (!functions.isEmpty()) {
                    ...
                    LOG.info("All Functions deleted from Database {}.", databaseName);
                }
            }
    ...
    public boolean isDatabaseEmpty(String databaseName) throws CatalogException {
        ...
        try {
            ...
            return (tableResponse.sdkHttpResponse().isSuccessful() && tableResponse.tableList().isEmpty()) &&
                    (functionResponse.sdkHttpResponse().isSuccessful() && functionResponse.userDefinedFunctions().isEmpty());
    ...
    public boolean tableExists(ObjectPath tablePath) throws CatalogException {
        checkNotNull(tablePath, "TablePath cannot be null.");
        return databaseExists(tablePath.getDatabaseName()) && glueTableOperator.glueTableExists(tablePath);
    }
    ...
    public boolean functionExists(ObjectPath path) throws CatalogException {
        checkNotNull(path, "Function path cannot be null.");
        ObjectPath functionPath = normalize(path);
        return databaseExists(functionPath.getDatabaseName()) && glueFunctionOperator.glueFunctionExists(functionPath);
    }
    ...
    public boolean isPartitionedTable(ObjectPath tablePath) {
        ...
        try {
            ...
            LOG.warn(tablePath.getFullName() + " Table doesn't Exists.", e);
            return false;
        }
    }

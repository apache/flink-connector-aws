...
class SplitTrackerTest {
    ...
    public void testMarkUnknownSplitAsFinishedIsNoop() {
        List<DynamoDbStreamsShardSplit> splits =
                Arrays.asList(
                        ...
                        getTestSplit(
                                generateShardId(6), Collections.singleton(generateShardId(3))));
        List<DynamoDBStreamsShardSplitWithAssignmentStatus> initialState =
                splits.stream()
                        .map(
                                split ->
    ...
    @Test
    public void testSplitTrackerCleansUpGarbageLeases() {
        String garbageShardId = "shardId-1234";
        String garbageShardId2 = "sharId-122345";

        List<Shard> shards =
                Arrays.asList(
                        generateShard(garbageShardId, "1000", "1500", null),
                        generateShard(1, "1200", "1300", null),
                        generateShard(2, "2000", null, null),
                        // shards produced by splitting parent shard
                        generateShard(3, "3100", null, generateShardId(0)),
                        generateShard(4, "4400", null, generateShardId(1)),
                        generateShard(garbageShardId2, "4100", null, garbageShardId));

        SplitTracker splitTracker = new SplitTracker(STREAM_ARN, InitialPosition.TRIM_HORIZON);
        splitTracker.addSplits(shards);

        splitTracker.markAsFinished(Arrays.asList(garbageShardId, garbageShardId2));
        ...
        assertThat(splitTracker.getKnownSplitIds())
                .doesNotContainAnyElementsOf(Arrays.asList(garbageShardId, garbageShardId2));
    }
    ...
    private DynamoDbStreamsShardSplit getSplitFromShard(Shard shard) {
        return new DynamoDbStreamsShardSplit(
                ...
                shard.parentShardId() == null
                        ...
                        : Collections.singleton(shard.parentShardId()));
    }
}

...
public class RedshiftDynamicTableFactory implements DynamicTableSinkFactory {
    ...
    public static final ConfigOption<Integer> PORT =
            ConfigOptions.key("port")
                    .intType()
                    .defaultValue(Integer.valueOf(5439))
    ...
    private RedshiftOptions getOptions(ReadableConfig config) {
        return (new RedshiftOptions.Builder())
                .withHostname((String) config.get(HOSTNAME))
                .withPort((int) config.get(PORT))
                ...
                .withCopyMode((boolean) config.get(COPY_MODE))
                .withTempS3Uri((String) config.get(TEMP_S3_URI))
                .withIamRoleArn((String) config.get(IAM_ROLE_ARN))
                .build();
    }
    ...
    private void validateConfigOptions(ReadableConfig config) {
        if (config.get(COPY_MODE) && !config.getOptional(TEMP_S3_URI).isPresent()) {
            ...
            String uri = (String) config.get(TEMP_S3_URI);
            try {

                RedshiftS3Util.getS3Parts(uri);
            } catch (Exception e) {
                throw new IllegalArgumentException(
                        String.format("A incorrect S3 URL {} privided!", uri), e);
            }
        }

        if (config.get(COPY_MODE) && !config.getOptional(IAM_ROLE_ARN).isPresent()) {
            throw new IllegalArgumentException(
                    "A IAM Role ARN which attached to the Reshift cluster must be provided as the COPY mode is True!");
        }
    }
}

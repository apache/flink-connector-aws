...
class GlueTableUtilsTest {
    ...
    void testColumnNameCaseSensitivity() {
        ...
        org.apache.flink.table.catalog.Column upperCaseColumn = org.apache.flink.table.catalog.Column.physical(
                "UpperCaseColumn", DataTypes.STRING()
        );
        
        org.apache.flink.table.catalog.Column mixedCaseColumn = org.apache.flink.table.catalog.Column.physical(
                "mixedCaseColumn", DataTypes.INT()
        );
        
        org.apache.flink.table.catalog.Column lowerCaseColumn = org.apache.flink.table.catalog.Column.physical(
        ...
        assertEquals("uppercasecolumn", glueUpperCase.name(), "Glue column name should be lowercase");
        assertEquals("mixedcasecolumn", glueMixedCase.name(), "Glue column name should be lowercase");
        assertEquals("lowercase_column", glueLowerCase.name(), "Glue column name should be lowercase");
        
        // 4. Verify that originalName parameter preserves case
        assertEquals("UpperCaseColumn", glueUpperCase.parameters().get("originalName"), 
                "originalName parameter should preserve original case");
        assertEquals("mixedCaseColumn", glueMixedCase.parameters().get("originalName"), 
                "originalName parameter should preserve original case");
        assertEquals("lowercase_column", glueLowerCase.parameters().get("originalName"), 
                "originalName parameter should preserve original case");
        ...
        assertEquals(3, columnNames.size(), "Schema should have three columns");
        assertTrue(columnNames.contains("UpperCaseColumn"), 
                "Schema should contain the uppercase column with original case");
        assertTrue(columnNames.contains("mixedCaseColumn"), 
                "Schema should contain the mixed case column with original case");
        assertTrue(columnNames.contains("lowercase_column"), 
    ...
    void testEndToEndColumnNameCasePreservation() {
        ...
        List<org.apache.flink.table.catalog.Column> flinkColumns = Arrays.asList(
                org.apache.flink.table.catalog.Column.physical("ID", DataTypes.INT()),
                org.apache.flink.table.catalog.Column.physical("UserName", DataTypes.STRING()),
                org.apache.flink.table.catalog.Column.physical("timestamp", DataTypes.TIMESTAMP()),
                org.apache.flink.table.catalog.Column.physical("DATA_VALUE", DataTypes.STRING())
        );
        
        // 2. Convert to Glue columns (simulating what happens in table creation)
        List<Column> glueColumns = flinkColumns.stream()
        ...
        for (int i = 0; i < flinkColumns.size(); i++) {
            ...
            assertEquals(originalName.toLowerCase(), glueName, 
                    "Glue column name should be lowercase of original");
            assertEquals(originalName, glueColumns.get(i).parameters().get("originalName"),
        ...
        List<String> resultColumnNames = schema.getColumns().stream()
                .map(col -> col.getName())
                .collect(Collectors.toList());
        
        for (org.apache.flink.table.catalog.Column originalColumn : flinkColumns) {
            String originalName = originalColumn.getName();
            assertTrue(resultColumnNames.contains(originalName),
                    "Result schema should contain original column name with case preserved: " + originalName);
        }
        ...
        String jsonExample = "{\"ID\":1,\"UserName\":\"test\",\"timestamp\":\"2023-01-01 12:00:00\",\"DATA_VALUE\":\"sample\"}";
        ...
        assertEquals("ID", resultColumnNames.get(0), "First column should maintain original case");
        assertEquals("UserName", resultColumnNames.get(1), "Second column should maintain original case");
        assertEquals("timestamp", resultColumnNames.get(2), "Third column should maintain original case");
        assertEquals("DATA_VALUE", resultColumnNames.get(3), "Fourth column should maintain original case");
    }
}

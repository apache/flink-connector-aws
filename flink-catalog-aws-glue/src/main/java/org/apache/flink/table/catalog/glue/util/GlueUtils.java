...
public class GlueUtils {
    ...
    public static void validateGlueResponse(GlueResponse response) throws CatalogException {
        ...
        for (org.apache.flink.table.resource.ResourceUri resourceUri : function.getFunctionResources()) {
            switch (resourceUri.getResourceType()) {
                ...
                case ARCHIVE:
                    ...
                    break;
                default:
                    throw new UnsupportedOperationException("GlueCatalog supports only creating resources JAR/FILE or ARCHIVE for now");
            }
    ...
    public void alterFunctionInGlue(ObjectPath functionPath, CatalogFunction newFunction) throws CatalogException {
    ...
    public Table getGlueTable(ObjectPath tablePath) throws TableNotExistException, CatalogException {
    ...
    public void createGlueFunction(ObjectPath functionPath, CatalogFunction function) throws CatalogException {
    ...
    public void removeGlueFunction(ObjectPath functionPath) throws CatalogException {
    ...
    public FunctionLanguage getFunctionalLanguage(UserDefinedFunction glueFunction) {
        if (glueFunction.functionName().equalsIgnoreCase(FunctionLanguage.JAVA.toString())) {
            return FunctionLanguage.JAVA;
        } else if (glueFunction.functionName().equalsIgnoreCase(FunctionLanguage.PYTHON.toString())) {
            return FunctionLanguage.PYTHON;
        } else if (glueFunction.functionName().equalsIgnoreCase(FunctionLanguage.SCALA.toString())) {
    ...
    public void ensurePartitionedTable(String catalogName, ObjectPath tablePath, Table glueTable)
    ...
    public void createGluePartition(
        ...
        createGluePartition(glueTable, partValues, sd.build(), properties);
    }
    ...
    public void createGluePartition(Table glueTable, List<String> partValues, StorageDescriptor sd, Map<String, String> properties) {

        // todo implement properly
        PartitionInput partitionInput = PartitionInput.builder().storageDescriptor(sd).parameters(properties).values(partValues).build();

        CreatePartitionRequest createPartitionRequest = CreatePartitionRequest.builder().partitionInput(partitionInput)
                .catalogId(awsProperties.getGlueCatalogId())
        ...
        try {
            ...
            throw new CatalogException("Glue Client failed to execute", e.getCause());
        }
    }

    public void alterGluePartition(ObjectPath tablePath, CatalogPartition existingPartition,
                                   CatalogPartition newPartition) throws CatalogException {
        PartitionInput partitionInput = PartitionInput.builder()
                .storageDescriptor(getPartitionStorageDescriptor(newPartition))
                .parameters(newPartition.getProperties())
                .lastAccessTime(Instant.now())
                .build();
        UpdatePartitionRequest updatePartitionRequest = UpdatePartitionRequest.builder()
                ...
                .build();
        try {
            UpdatePartitionResponse response = glueClient.updatePartition(updatePartitionRequest);
            validateGlueResponse(response);
        } catch (GlueException e){
            throw new CatalogException("Glue Client failed to execute", e.getCause());
        }
    ...
    private StorageDescriptor getPartitionStorageDescriptor(CatalogPartition partition) {
        return StorageDescriptor.builder().parameters(partition.getProperties()).build();
    }

    public void dropGluePartition(ObjectPath tablePath, CatalogPartitionSpec partitionSpec)
            throws CatalogException {
        DeletePartitionRequest deletePartitionRequest = DeletePartitionRequest.builder()
                .catalogId(awsProperties.getGlueCatalogId())
                .databaseName(tablePath.getDatabaseName())
                .tableName(tablePath.getObjectName())
                .partitionValues(partitionSpec.getPartitionSpec().values())
                .build();
